...
class TConvTransE(torch.nn.Module):
    def __init__(self, num_entities, embedding_dim, input_dropout=0, hidden_dropout=0, feature_map_dropout=0, channels=50, kernel_size=3, sequence_len = 10, use_bias=True):
        ...
        super(TConvTransE, self).__init__()
        ...
        self.tconv_list = torch.nn.ModuleList()
    ...
    def forward(self, embedding, emb_rel_list, triplets, nodes_id=None, mode="train", negative_rate=0, partial_embeding=None):
        score_list = []
        batch_size = len(triplets)
        for idx in range(len(embedding)):
            e1_embedded_all = F.tanh(embedding[idx])
            emb_rel = F.tanh(emb_rel_list[idx])
